 ...
 */
ElasticSearchClient.prototype.discoverNodes = function(callback){
    var self = this;
    ...
    query.on('data', function(data){
        ...
        try {
            ...
            if (callback) {
                callback ({msg : e.message, stack : e.stack});
            }
            return;
        }
        ...
        if(hosts.length > 0){
            ...
            // set secure = true if in prod (this is a hack)
            self.clientOptions.hosts.forEach (function (node) {
                if (process.env.NODE_ENV === 'production' ) {
                    node.secure = true;
                    node.port = 9201;
                }
            });
            console.log ('hosts', self.clientOptions.hosts);
        }
    ...
    query.on ('error', function (err) {
        console.error ('could not discover nodes', err);
...
ElasticSearchClient.prototype.getNodes = function(){
    return self.clientOptions.hosts;
}